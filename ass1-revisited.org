...
** <<<QR-01>>> : /Security/
   + Definition :: A Service is /secure/ when the users of the service
                   can access only those things they are intended to
                   access, and the system is not susceptible to
                   malicious external actors.
** <<<QR-02>>> : /Availability/
   + Definition :: A Service is considered /available/ if there is
                   little or no time when it cannot be accessed, under
                   the intended conditions.
** <<<QR-03>>> : /Reliability/
   + Definition :: A Service is /reliable/ when it routinely produces
                   the expected and desired result, within some margin
                   of tolerance.
** <<<QR-04>>> : /Usability/

#+latex: \newpage
* Business Services
** <<<BS-01>>> : /Transaction Monitoring/
*** Service Behavior
